...
def generate_ai_summary_placeholder(findings: list[dict], table_rows: list[dict]) -> str:
    ...
    lines.append("Top SAE narrative review signals:")
    for finding in findings[:3]:
        lines.append(f"- {finding['title']} ({finding['risk_level']})")
    if table_rows:
        lines.append("")
        lines.append("Detected SAE completeness context:")
        for row in table_rows[:2]:
            lines.append(f"- {row['Narrative Element']} | {row['Status']} | {row['Evidence']}")
    lines.append("")
...
def generate_ai_summary(text: str, findings: list[dict], table_rows: list[dict]) -> str:
    ...
    table_text = "\n".join(
        [
            f"- {row['Narrative Element']} | {row['Status']} | {row['Evidence']} | {row['Suggested Review Direction']}"
            for row in table_rows
        ]
    ) or "- No SAE completeness rows available."

    client = anthropic.Anthropic(api_key=api_key)

    system_prompt = (
        "You are an assistive SAE narrative completeness summarizer. "
        "You must summarize only the deterministic findings and SAE completeness table provided to you, plus limited clearly grounded interpretation that directly follows from those findings. "
        "Do not invent new findings. "
        "Do not state PASS or FAIL. "
        "Do not state the narrative is submission-ready, medically adequate, or regulatorily complete unless that was already explicitly established in the deterministic findings. "
        "Do not make definitive legal, medical, clinical, pharmacovigilance, or regulatory judgments. "
...
Rules:
- Do not add new findings not supported by the deterministic findings or SAE completeness table.
- Do not use bullet points.
- Do not output PASS or FAIL.
- Do not say the narrative is submission-ready, medically adequate, or regulatorily complete.
...
def build_transparency_report_text(text_input: str = "", findings=None, table_rows=None) -> str:
    ...
    lines = [
        ...
        f"- SAE completeness rows detected: {len(table_rows)}",
        "",
        "3. HOW THE SYSTEM WORKS",
        "- The deterministic engine checks SAE narrative text against a fixed set of completeness heuristics.",
        "- The AI layer summarizes deterministic findings and SAE completeness table outputs in plain language.",
        "- Human review remains required.",
        "",
        "4. SAE NARRATIVE LOGIC USED IN THIS DEMO",
        "- Patient-context detection",
        "- Event-description detection",
        "- Exposure / suspect-product context detection",
        "- Chronology / onset detection",
        "- Seriousness / outcome detection",
        "- Action-taken detection",
        "- Follow-up / rechallenge context detection",
        "- SAE completeness table generation",
        "",
        "5. CURRENT SNAPSHOT",
        ...
        "- It does not determine MedWatch/EudraVigilance submission readiness, medical causality, or final reporting adequacy.",
        "- It does not provide legal, medical, clinical, pharmacovigilance, or regulatory approval.",
...
def build_transparency_report_pdf(text_input: str = "", findings=None, table_rows=None) -> bytes:
    ...
    c.setTitle("SAE-Narrative-Scorer Transparency & Oversight Report")
    c.setFont("Helvetica-Bold", 14)
    c.drawString(left, y, "SAE-Narrative-Scorer Transparency & Oversight Report")
    y -= 10 * mm
...
if st.session_state.sae_done:
    ...
    st.download_button(
        ...
        file_name="sae_narrative_scorer_transparency_report.pdf",
    ...
    if allowed:
        if st.button("Generate AI summary"):
            st.session_state.sae_ai_notice_open = True
            st.rerun()
        else:
            st.caption("AI summary is available for this input under current public-demo limits.")

        if st.session_state.get("sae_ai_notice_open"):
            render_ai_data_notice()
            col_ai_1, col_ai_2 = st.columns(2)
            with col_ai_1:
                if st.button("No, I don't want to use AI", key="sae_ai_notice_cancel"):
                    st.session_state.sae_ai_notice_open = False
                    st.rerun()
            with col_ai_2:
                if st.button("Yes, I understand and confirm the submitted text is example, synthetic, public, or fully anonymised.", key="sae_ai_notice_confirm"):
                    with st.spinner("Generating AI summary..."):
                        try:
                            st.session_state.sae_ai_summary = generate_ai_summary(last_text, findings, table_rows)
                            st.session_state.sae_ai_notice_open = False
                            st.rerun()
        ...
        if st.session_state.sae_ai_summary:
            render_structured_ai_summary(st.session_state.sae_ai_summary)
    else:
