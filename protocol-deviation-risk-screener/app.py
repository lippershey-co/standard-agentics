...
def looks_like_protocol_deviation_scope(text: str) -> bool:
...
def ai_summary_allowed(text: str):
    ...
    if not looks_like_protocol_deviation_scope(text):
        return False, "This public demo AI summary is limited to protocol, study-design, or deviation-risk review text. For broader document review or custom workflows, contact us for pricing."
    return True, ""
...
def generate_ai_summary_placeholder(findings: list[dict], table_rows: list[dict]) -> str:
    ...
    lines.append("Top deviation-risk review signals:")
    for finding in findings[:3]:
        lines.append(f"- {finding['title']} ({finding['risk_level']})")
    if table_rows:
        lines.append("")
        lines.append("Detected deviation-risk context:")
        for row in table_rows[:2]:
            lines.append(f"- {row['Study Title']} | {row['Risk Type']} | {row['Objective Context']}")
    lines.append("")
...
def generate_ai_summary(text: str, findings: list[dict], table_rows: list[dict]) -> str:
    ...
    table_text = "\n".join(
        [
            f"- {row['Study Title']} | {row['Risk Type']} | {row['Objective Context']} | {row['Evidence']} | {row['Suggested Review Direction']}"
            for row in table_rows
        ]
    ) or "- No deviation-risk rows available."

    client = anthropic.Anthropic(api_key=api_key)

    system_prompt = (
        "You are an assistive protocol deviation-risk summarizer. "
        "You must summarize only the deterministic findings and deviation-risk table provided to you, plus limited clearly grounded interpretation that directly follows from those findings. "
        "Do not invent new findings. "
        "Do not state PASS or FAIL. "
        "Do not state the protocol is GLP-compliant, audit-ready, or regulatorily acceptable unless that was already explicitly established in the deterministic findings. "
        "Do not make definitive legal, medical, clinical, audit, or regulatory judgments. "
...
Rules:
- Do not add new findings not supported by the deterministic findings or deviation-risk table.
- Do not use bullet points.
- Do not output PASS or FAIL.
- Do not say the protocol is GLP-compliant, audit-ready, or regulatorily acceptable.
...
def build_transparency_report_text(text_input: str = "", findings=None, table_rows=None) -> str:
    ...
    lines = [
        ...
        "",
        "1. SYSTEM IDENTITY",
        "- System: Protocol-Deviation-Risk-Screener",
        ...
        f"- Deviation-risk rows detected: {len(table_rows)}",
        "",
        "3. HOW THE SYSTEM WORKS",
        "- The deterministic engine checks protocol-style text against a fixed set of deviation-risk heuristics.",
        "- The AI layer summarizes deterministic findings and deviation-risk table outputs in plain language.",
        "- Human review remains required.",
        "",
        "4. DEVIATION-RISK LOGIC USED IN THIS DEMO",
        "- Control-arm structure detection",
        "- Objective-endpoint alignment signal detection",
        "- Operational ambiguity detection",
        "- Deviation-handling language detection",
        "- Documentation / GLP signal detection",
        "- Deviation-risk table generation",
        "",
        "5. CURRENT SNAPSHOT",
        ...
        "- It does not determine formal GLP compliance, audit outcome, or final regulatory acceptability.",
        "- It does not provide legal, medical, clinical, audit, or regulatory approval.",
...
def build_transparency_report_pdf(text_input: str = "", findings=None, table_rows=None) -> bytes:
    ...
    c.setTitle("Protocol-Deviation-Risk-Screener Transparency & Oversight Report")
    c.setFont("Helvetica-Bold", 14)
    c.drawString(left, y, "Protocol-Deviation-Risk-Screener Transparency & Oversight Report")
    y -= 10 * mm
...
if st.session_state.pdr_done:
    ...
    st.download_button(
        ...
        file_name="protocol_deviation_risk_screener_transparency_report.pdf",
    ...
    if allowed:
        if st.button("Generate AI summary"):
            st.session_state.pdr_ai_notice_open = True
            st.rerun()
        else:
            st.caption("AI summary is available for this input under current public-demo limits.")

        if st.session_state.get("pdr_ai_notice_open"):
            render_ai_data_notice()
            col_ai_1, col_ai_2 = st.columns(2)
            with col_ai_1:
                if st.button("No, I don't want to use AI", key="pdr_ai_notice_cancel"):
                    st.session_state.pdr_ai_notice_open = False
                    st.rerun()
            with col_ai_2:
                if st.button("Yes, I understand and confirm the submitted text is example, synthetic, public, or fully anonymised.", key="pdr_ai_notice_confirm"):
                    with st.spinner("Generating AI summary..."):
                        try:
                            st.session_state.pdr_ai_summary = generate_ai_summary(last_text, findings, table_rows)
                            st.session_state.pdr_ai_notice_open = False
                            st.rerun()
        ...
        if st.session_state.pdr_ai_summary:
            render_structured_ai_summary(st.session_state.pdr_ai_summary)
    else:
